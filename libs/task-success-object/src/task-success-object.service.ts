...
import {
  TaskSuccessObject,
  TaskSuccessObjectDocument,
} from './entity/task-success-object-entity';
...
export class TaskSuccessObjectService implements ITaskSuccessObjectService {
  ...
  async createTaskSuccessObject(
    payload: CreateTaskSuccessObject,
  ): Promise<any> {
  ...
  async findCountSuccessByShortcode(
    ...
    return await this.taskSuccessObjectModel
  ...
  async findTaskSuccessObjectByProjectId(
    payload: FindCountTaskSuccessObjectByProject,
  ): Promise<any> {
  ...
  async findCountTaskSuccessObjectByProjectId(
    payload: FindCountTaskSuccessObjectByProject,
  ): Promise<any> {
    const { project_id } = payload;
    return await this.taskSuccessObjectModel
  ...
  async updateRemoveDecimal() {
    const taskSuccess = await this.taskSuccessObjectModel.find({
      project_id: '613761969a78147603abb475',
      user_id: '6151ebb42c34ce153c049d76',
    });
    for (const [index, iterator] of taskSuccess.entries()) {
      ...
      await this.taskSuccessObjectModel.updateOne(
        { _id },
        { object: updateRemoveDecimal },
      );
  ...
  async updatePayment() {
    await this.taskSuccessObjectModel.updateMany(
      { project_id: '613761969a78147603abb475' },
      { payment: 'success', paymentAt: new Date() },
    );
  }
}
